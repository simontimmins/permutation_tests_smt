...
vcf=/exports/igmm/eddie/semple-lab/stimmins/gnomad_v4/seg_snvs/cpg_tpg/rev_cpg_tpg_both_${chrom}_snvs.bed
unstarch ${vcf%.bed*}.starch > ${vcf}
conda deactivate
name=${peak_name}_fps_cpg_tpg_SNVs_chr1
output=$name'.pdf'
echo $vcf
echo $name
echo $atac
...
#remove uncompressed, ONLY do if also a starch
rm $vcf
